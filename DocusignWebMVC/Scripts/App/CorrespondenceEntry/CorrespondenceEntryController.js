new function () {
    ...
    App.controller('CorrespondenceEntryController', function ($scope, $http, $location, $filter, ngDialog) {
        ...
        var initParams = {
            dbEnvironment: dbEnvironment, litAbbr: $scope.LitAbbr
        }

        $http.get("/MassEntry/GetCorrespondenceEntryInitialValues", { params: initParams })
        .success(function (response, status) {
            $scope.ActivityTypes = response.ActivityTypes;
            $scope.ActivityDate = response.ActivityDate;
        });

        $scope.UploadCorrespondenceEntries = function () {
            ...
            if (!confirm("YOU ARE ABOUT TO UPLOAD " + noOfWLNumbers + " CORRESPONDENCE ENTRIES. ARE YOU SURE YOU WANT TO PROCEED?")) {
                return false;
            }
            ...
            $http.post("/MassEntry/UploadCorrespondenceEntries", {
                jsonData: jsonData
